...
export function buildPermissionEmailDraft({ employee, manager, hrApprover, request, employees = [] }) {
  ...
  const body = [
    `Dear ${firstName},`,
    ``,
    `Your request for a ${typeBoth.en.toLowerCase()} permission on ${fmtDateShort(request.permission_date)}${win} has been approved.`,
    ``,
    `Reason on file, as mentioned by you: ${request.reason || '—'}.`,
    ``,
    `Kindly find the signed permission letter attached. Please print the document, obtain your manager's signature, and submit the hard copy to the HR office.`,
    ``,
    `— Company Policy Reminder | تنبيه بسياسة الشركة —`,
    ``,
    `  • Maximum of three (3) permissions per calendar month — late arrival or early departure.`,
    `    ثلاث استئذانات كحد أقصى في الشهر، تأخير أو انصراف مبكر.`,
    ``,
    `  • Each request shall not exceed sixty (60) minutes.`,
    `    ولا يتجاوز الاستئذان الواحد ستين دقيقة.`,
    ``,
    `  • Kindly observe these limits; any exceedance may be reviewed by HR.`,
    `    نأمل التكرم بالالتزام بهذه الحدود، وأي تجاوز قد يُراجع من قِبَل الموارد البشرية.`,
    ``,
    `Thanks and regards,`,
    ``,
    HR_SIGNATURE.name,
