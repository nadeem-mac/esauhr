...
export default function Dashboard({ me, employees, requests, typeMap, empMap, permissions, leaveTypes, onGoToRequests, onGoToReviews, onNewRequest, onOpenOrgChart, onUploadCert }) {
  ...
  // Headcount block (number/table style) — defined once so it can render
  // either inside Bashaier's at-a-glance hub or standalone for admin.
    ...
    <details className="group">
      <summary className="flex items-baseline justify-between flex-wrap gap-3 mb-5 cursor-pointer list-none"
        ...
        <div>
          ...
          <h2 className="serif" style={{ fontSize: '20px', color: '#1F1B16', marginTop: '4px', fontWeight: 500 }}>
            Headcount by department
            <span className="ml-3 text-[13px]" style={{ color: '#1F1B16', opacity: 0.5, fontWeight: 400 }}>
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Hero — non-Bashaier only. Bashaier's greeting/affirmation now
          lives in the global header by her name (Nadeem 2026-06-12);
          her landing goes straight into the consolidated hub below. */}
      {!bashaierMode && (
