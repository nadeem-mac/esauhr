 ...
 */
export default function ShiftStaffAttendanceReportCard({ employees = [], me, compact = false }) {
  ...
  // ─── Compact "Morning report (John)" mode (Reports tab) ──────────────
  // Derives the three body sections the email/Outlook copy show:
  //   • today's late arrivals (judged from check-in, 15-min grace)
  //   • yesterday's early departures + missed punches
  ...
  const morningData = useMemo(() => {
    ...
    return { late, early, missed, shift };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const copyMorningHtml = async () => {
    ...
    // Same roll-call counts + sheet labels the mailto email uses, so both
    // read identically. (Nadeem 2026-06-13)
    const clock = new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });
    const wkShort = (iso) => new Date(iso + 'T00:00:00').toLocaleDateString('en-GB', { weekday: 'short' });
    const dShort  = (iso) => new Date(iso + 'T00:00:00').toLocaleDateString('en-GB', { day: '2-digit', month: 'short' });
    const dayDate = (iso) => new Date(iso + 'T00:00:00').toLocaleDateString('en-GB', { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' });
    const sheet1 = `Today (${dShort(t)}, ${wkShort(t)})`.slice(0, 31);
    const sheet2 = `Yesterday detail (${dShort(y)}, ${wkShort(y)})`.slice(0, 31);
    let inCount = 0, lateCount = 0, notIn = 0, leaveCount = 0;
    reportSummaries.forEach(s => {
    ...
    const buildDetailTable = (rows) => {
      if (!rows.length) return `<p style="margin:4px 0 12pt 0;color:#6B7280;font-size:10pt">None.</p>`;
      let n = 0;
  ...
  ${renderHrSignatureHtml()}
    ...
    try {
      ...
      setMorningCopied(true);
      setTimeout(() => setMorningCopied(false), 2000);
    } catch (e) {
      alert('Could not copy to clipboard: ' + (e?.message || e));
    }
  };

  // Compact mode auto-activates: load today + last working day on mount,
  // and re-load whenever a daily time card is uploaded (AttendanceView
  // dispatches 'esau:attendance-changed' after each upload write).
  useEffect(() => {
    if (!compact) return;
    const run = () => {
      const t = todayLocal();
  ...
  // ─── render ──
  if (compact) {
    const ACCENT = me?.id === 'H94830' ? '#993556' : '#0F4C2A';
    const ACCENT_BG = me?.id === 'H94830' ? '#FBEAF0' : '#ECFDF5';
    const ACCENT_BD = me?.id === 'H94830' ? '#F4C0D1' : '#A7F3D0';
    const todayHas = attendance.some(r => r.attendance_date === to);
    const active = searched && !loading && todayHas;
    const clock = new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });
    const btn = (extra) => ({
      height: 38, minWidth: 150, padding: '0 16px', borderRadius: 8,
      fontSize: 12, fontWeight: 700, display: 'inline-flex', alignItems: 'center',
      justifyContent: 'center', gap: 6, cursor: 'pointer', border: '1px solid transparent',
      ...extra,
    });
    const MiniTable = ({ title, heads, rows, render }) => (
      <div className="rounded-lg border bg-white p-3" style={{ borderColor: 'var(--border-soft)' }}>
        <div className="text-[11px] font-bold mb-2" style={{ color: ACCENT, letterSpacing: '0.03em' }}>
          {title} <span style={{ color: '#1F1B16', opacity: 0.5, fontWeight: 500 }}>· {rows.length}</span>
        </div>
        {rows.length === 0 ? (
          <div className="text-[11px]" style={{ color: '#1F1B16', opacity: 0.5 }}>None.</div>
        ) : (
          <table className="w-full text-[11px]" style={{ color: '#1F1B16' }}>
            <thead><tr style={{ opacity: 0.6 }}>{heads.map(h => <th key={h} className="text-left font-semibold py-1">{h}</th>)}</tr></thead>
            <tbody>{rows.map((r, i) => <tr key={i} className="border-t border-black/5">{render(r)}</tr>)}</tbody>
          </table>
        )}
      </div>
    );
